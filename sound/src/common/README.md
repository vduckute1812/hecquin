...
| `Utf8.hpp` | Header-only. `sanitize_utf8(input) → std::string` — replaces invalid / overlong sequences with U+FFFD (or a configurable replacement), skips CP-1252 bytes like 0xA0 that would otherwise crash downstream JSON parsers.<br>Plus the `utf8::` sub-namespace shared by chunkers + IPA tokenizers: `codepoint_length(byte)`, `is_continuation(byte)`, `align_to_codepoint(view, pos)`. Replaces the byte-level UTF-8 walks that used to live duplicated in `learning/TextChunker.cpp` and `learning/pronunciation/PhonemeVocab.cpp`. |

...
| `StoreApiSink.hpp/cpp` | — | `make_store_api_call_sink(LearningStore&) → ApiCallSink` factory plus `compose_sinks(a, b)` for fanning records into multiple sinks. Both `EnglishIngest` and `EnglishTutorMain` plug the resulting lambda into `LoggingHttpClient` so every embedding / chat call lands in `documents` API-call telemetry. Single source of truth for the lambda body. |

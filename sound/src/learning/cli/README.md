...
| `StoreApiSink.hpp/cpp` | — | `make_store_api_call_sink(LearningStore&) → ApiCallSink` factory plus `compose_sinks(a, b)` for fanning records into multiple sinks. Both `EnglishIngest` and `EnglishTutorMain` plug the resulting lambda into `LoggingHttpClient` so each embedding / chat HTTP request is recorded in the `api_calls` table (when the store opened). Single source of truth for the lambda body. |
| `CsvApiCallSink.hpp/cpp` | — | `make_csv_api_call_sink(path) → ApiCallSink` factory. Appends one CSV row per outbound `/embeddings` POST so cost can be summed in a spreadsheet. Off when `path` is empty. RFC-4180 quoting for embedded `,`/`"`. Used by `english_ingest`. |
| `RunSummaryCsv.hpp/cpp` | — | `append_run_summary(path, RunSummaryRow)` writer. Adds one CSV row per `Ingestor::run()` (totals + duration + exit code). Header is written once on first call. |
| `CsvEscape.hpp` | header-only | `csv_escape(field)` helper shared by the two CSV writers above. |
| `EnglishIngest.cpp` | `english_ingest` | Curriculum → chunks → embeddings → SQLite. Thin `main()` that constructs an `Ingestor` and prints the resulting `IngestReport`. Uses `StoreApiSink` + (optional) `CsvApiCallSink` / `RunSummaryCsv` for telemetry + spend logging, `cli/DefaultPaths.hpp` for path defaults. CSV outputs are gated by `HECQUIN_INGEST_API_LOG_CSV` / `HECQUIN_INGEST_RUN_SUMMARY_CSV` (or `--api-log-csv` / `--run-summary-csv`). |
